...
void shuffle(Player member[], Player& dealer)
{
    ...
    for (int i = 5; i < 7 && member[0].checkSurvivor() == false; i++)
    {
        R1[a] = R_base[i];
        S1[a] = S_base[i];
        member[0].getCardSR(R1[a], S1[a], 2);
        a++;
    }
    //플레이어 2 카드 2장
    for (int i = 7; i < 9 && member[0].checkSurvivor() == false; i++)
    {
        R2[b] = R_base[i];
        S2[b] = S_base[i];
        member[1].getCardSR(R2[b], S2[b], 2);
        b++;
    }
    //플레이어 3 카드 2장
    for (int i = 9; i < 11 && member[0].checkSurvivor() == false; i++)
    {
        R3[c] = R_base[i];
        S3[c] = S_base[i];
        member[2].getCardSR(R3[c], S3[c], 2);
        c++;
    }
    //플레이어 4 카드 2장
    for (int i = 11; i < 13 && member[0].checkSurvivor() == false; i++)
    {
        R4[d] = R_base[i];
        S4[d] = S_base[i];
        member[3].getCardSR(R4[d], S4[d], 2);
        d++;
    }
    //플레이어 5 카드 2장
    for (int i = 13; i < 15 && member[0].checkSurvivor() == false; i++)
    {
        R5[e] = R_base[i];
        S5[e] = S_base[i];
        member[4].getCardSR(R5[e], S5[e], 2);
        e++;
    }
}
